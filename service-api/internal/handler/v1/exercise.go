...
func (h *ExerciseHandler) List(c *gin.Context) {
	var req models.ExerciseListRequest
...
func (h *ExerciseHandler) Get(c *gin.Context) {
	var req models.ExerciseGetRequest
	if err := c.ShouldBindUri(&req); err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": "invalid uri: " + err.Error()})
		return
	}
	...
	res, err := h.service.Get(c.Request.Context(), req, include)
	if err != nil {
		switch {
		//Validation error
		case errors.Is(err, models.ErrValidation):
...
func (h *ExerciseHandler) Delete(c *gin.Context) {
	var req models.ExerciseDeleteRequest
	if err := c.ShouldBindJSON(&req); err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": "invalid JSON: " + err.Error()})
		return
	}
